...
describe('todo model', () => {
  it('returns an empty list initially', async (t) => {
    ...
    assert.deepEqual(await model.list(), {items: [], totalCount: 0, completedCount: 0})
  })
  ...
  it('persists todos across model instances', async (t) => {
    ...
    const {items: todos} = await fresh.list()
  ...
  it('updates fields of a todo', async (t) => {
    ...
    const updated = await model.update(todo.id, {title: 'new', completed: true, notes: undefined})
  ...
  it('returns undefined when updating a non-existent todo', async (t) => {
    ...
    assert.equal(
      await model.update(999, {title: 'x', notes: undefined, completed: undefined}),
      undefined,
    )
  })
  ...
  it('removes a todo and returns true', async (t) => {
    ...
    const {items: remaining} = await model.list()
